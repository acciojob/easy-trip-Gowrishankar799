...
public class AirportRepository {
    ...
    public void addAirport(Airport airport){
        String s = airport.getAirportName();
        airportHashMap.put(s,airport);
    }
    public String getLargestAirportName(){
        int max = 0;
        for(Airport a : airportHashMap.values()){
            max = Math.max(max,a.getNoOfTerminals());
        }
        List<String>list = new ArrayList<>();
        for(Airport a : airportHashMap.values()){
           if(max == a.getNoOfTerminals()){
               list.add(a.getAirportName());
           }
        }
        Collections.sort(list);
        return list.get(0);
    }
    public double getduration(City from,City to){
